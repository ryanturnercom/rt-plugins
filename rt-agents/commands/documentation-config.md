...
Files discovered in `docs/`, `documentation/`, `wiki/`, or `guides/` that share the same immediate subfolder MUST be grouped into one section. For example, `docs/api/overview.md` and `docs/api/endpoints.md` become:

```toml
[[section]]
title = "API"
sources = ["docs/api/overview.md", "docs/api/endpoints.md"]
mode = "verbatim"
print = true
order = 400
```

Files directly in the root of a documentation folder (not in a subfolder) remain as individual sections.
...
Combine all discovered blueprint file paths into one `sources` array.
...
# Auto-generated by /documentation-config
# Last scanned: <YYYY-MM-DD HH:MM:SS>
#
# Edit this file to customize documentation generation.
# Re-run /documentation-config to rescan and merge new sources.
...
> When you are satisfied with the configuration, run `/documentation-create` to generate the full documentation.
